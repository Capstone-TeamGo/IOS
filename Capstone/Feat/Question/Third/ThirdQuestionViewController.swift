    ...
    private func setBinding() {
        ...
        reissueViewModel.reissueExpire
            ...
            if expire == true {
                ...
                DispatchQueue.main.async {
                    guard let audioURL = URL(string: self.question.data?.accessUrls?[2] ?? "") else {return}
                    self.questionText.text = self.question.data?.questionTexts?[2]
                    self.player = AVPlayer(url: audioURL)
                    self.player?.play()
                }
                self.mic.rx.tap
                ...
                self.nextBtn.rx.tap
                    .subscribe { _ in
                        self.voiceRecordViewModel.postRecordTrigger.onNext([self.question.data?.analysisId ?? 0, self.question.data?.questionIds?[2] ?? 0])
                        DispatchQueue.main.async {
